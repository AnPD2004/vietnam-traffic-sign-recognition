...
def write_yolo_ref(best_dir: Path, yolo_best: dict[str, Any], source_path: Path) -> Path:
    payload = {
        "ref": source_path.as_posix(),
        "run_id": yolo_best.get("run_id"),
        "weights": yolo_best.get("weights"),
        "hyperparams": yolo_best.get("hyperparams", {}),
        "metrics": yolo_best.get("metrics", {}),
    }
    path = best_dir / "yolo_ref.json"
    path.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
    return path
...
def finalize_best(
    ...
    write_yolo_ref(best_dir, yolo_best, yolo_source)
