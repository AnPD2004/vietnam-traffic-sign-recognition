...
def generate_report(output_root: Path, registry: dict[str, Any], mode: str) -> None:
    ...
    for experiment, run_id in registry.get("best_per_experiment", {}).items():
        ...
        metric = entry.get("metrics", {}).get("map50_95")
        lines.append(f"- **{experiment}**: `{run_id}` (mAP50-95={metric})")
    ...
    for run_id in sorted(runs):
        ...
        lines.append(f"- `{run_id}` — {status}, mAP50-95={metric}{suffix}")
...
def _write_experiment_report(
    ...
    lines = [f"# {experiment}", "", f"Best: `{best_run_id}`", "", "| run_id | mAP50-95 | status |", "|--------|----------|--------|"]
    for run_id, entry in sorted(experiment_runs):
        metric = entry.get("metrics", {}).get("map50_95")
        status = entry.get("status")
        lines.append(f"| `{run_id}` | {metric} | {status} |")
