...
from src.pipeline1.config import (
    apply_test_mode,
    data_yaml_path,
    experiment_order,
    load_config,
...
from src.pipeline1.experiments import build_runs_for_experiment, state_from_params
from src.pipeline1.report import generate_report
from src.pipeline1.run_one import build_run_spec_dict, write_job_file
from src.pipeline1.select_best import finalize_best, select_experiment_best, write_experiment_best
from src.pipeline1.train import utc_now, write_metrics, write_run_config
...
def _print_run_plan(specs: list[RunSpec], mode: str) -> None:
    print(f"=== PIPELINE 1 PLAN ({mode}) — {len(specs)} runs ===")
...
def _run_in_subprocess(
    ...
    *,
    ...
    """Spawn a fresh Python process per run so RAM/GPU is fully released on exit."""
    ...
    try:
        ...
        cmd = [sys.executable, "-m", "src.pipeline1.run_worker", "--job", str(job_path)]
        # Inherit stdio — Ultralytics logs use UTF-8; capturing breaks on Windows cp1252.
...
def _run_in_process(
    ...
    *,
    ...
    from src.pipeline1.run_one import execute_single_run
...
def run_pipeline(flags: PipelineFlags) -> int:
    ...
    registry = load_registry(registry_path, pipeline="pipeline1", mode=mode)
    registry["pipeline"] = "pipeline1"
    registry["mode"] = mode

    if flags.list_runs:
        runs = registry.get("runs", {})
        print(f"=== PIPELINE 1 RUNS ({mode}) — {len(runs)} entries ===")
        for run_id in sorted(runs):
            ...
            print(
                ...
                f"mAP50-95={entry.get('metrics', {}).get('map50_95')}"
            )
        return 0
    ...
    for experiment in run_experiments:
        ...
        for params in param_list:
            ...
            exp_specs.append(
                RunSpec(
                    ...
                    / weights_filename(run_id, pipeline="pipeline1"),
                )
            )
        ...
        metric = cfg["experiments"][experiment].get("metric", "map50_95")
        run_spec_dicts = [
            {
                "run_id": s["run_id"],
    ...
    save_registry(registry_path, registry)
    best_json = finalize_best(out_root, final_best, mode)
    generate_report(out_root, registry, mode)

    print(f"\n=== PIPELINE 1 COMPLETE ({mode}) ===")
    print(f"Best: {final_best['run_id']}")
...
def parse_args(argv: list[str] | None = None) -> PipelineFlags:
    parser = argparse.ArgumentParser(description="Pipeline 1 — YOLO end-to-end benchmark")
    parser.add_argument("--config", default="configs/pipeline1.yaml", help="Config YAML path")
    ...
    parser.add_argument("--skip-eval", action="store_true", help="Skip validation/FPS after train")
